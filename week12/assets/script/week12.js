...
const makeTen = () => {
	...
	console.log(`Вторая буква: ${secondLetter}`); //Вторая буква: e
};
...
const makeEleven = () => {
	resultEleven.innerHTML = Math.abs(5, 2);
};
...
const makeEighteen = () => {
	...
	const res = str.replace('world', 'JavaScript');
	console.log(res);
};
...
const makeNineteen = () => {
	const str = 'Hello, world!';
	const res = str.substring(7, str.length)
	console.log(res);
};
...
const makeTwentySix = () => {
	...
	const res = randomNumber.toFixed(4);
	console.log(res);
};
...
const makeTwentyEight = () => {
	const ceilNumber = Math.ceil(4.47);
